...
function handleevent($event,&$objcliente,  &$objCMSUser) 
{
	switch ($event) {
		case "index":
			...
			$dataArr = Array();
			$result=$objCMSUser->doformat("/alarmas/index.html", $_SESSION["TEMPLATE"]["TEMPLATE_ROOT"]);
			...
			case "view":
				...
				$opts = array("include_docs" => true,"startkey" => array($_SESSION["ACCOUNT"]["NAME"],"ALARMA","REGISTER"), "endkey" => array($_SESSION["ACCOUNT"]["NAME"],"ALARMA","REGISTER"), "limit" => 100 , "descending" => true);
				...
				try {
					...
					$dataArr = $objcliente->setQueryParameters($opts)->getView('alarmas','by_cuenta');
				} catch (Exception $e) {
...
//echo $_SESSION["MYSQL"]["HOST"]."-".$_SESSION["MYSQL"]["NAME"]."-".$_SESSION["MYSQL"]["USER"]."-".$_SESSION["MYSQL"]["PASSWORD"];
...
$tplmarco->setVar("{page.title}","Pignus Alarmas - Quodii SAS");
