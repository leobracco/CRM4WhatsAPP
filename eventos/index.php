...
function handleevent($event,&$objcliente,  &$objCMSUser) 
{
	switch ($event) {
		case "index":
			if (!$objCMSUser->checkPermission("Eventos::lectura"))
				return handleevent("permission_denied",  $objCMSUser);
			$dataArr = Array();
			$result=$objCMSUser->doformat("/eventos/index.html", $_SESSION["TEMPLATE"]["TEMPLATE_ROOT"]);
...
$tplmarco->setVar("{page.title}","Pignus Eventos - Quodii SAS");
